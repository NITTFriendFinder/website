...
def prev_papers():
    if request.method == "GET":
        # final = []
        mycursor.execute("SELECT DISTINCT dept FROM question_paper")
        dept = list(mycursor.fetchall())
        mycursor.execute("SELECT DISTINCT courseid FROM question_paper")
        courseid = list(mycursor.fetchall())
        mycursor.execute("SELECT DISTINCT exam FROM question_paper")
        exam = list(mycursor.fetchall())
    ...
    if request.method == "POST":
        ...
        while i<len(final):
            if sem!="blank" and sem != final[i][1]:
                final.pop(i)
                continue
            if dept!="blank" and dept != final[i][2]:
                final.pop(i)
                continue
            if courseid!="blank" and courseid != final[i][3]:
                final.pop(i)
                continue
            if exam!="blank" and exam != final[i][4]:
                final.pop(i)
                continue
            if year!="blank" and year != final[i][5]:
                final.pop(i)
                continue
            if prof!="blank" and prof not in final[i][6]:
                final.pop(i)
                continue
            i+=1
